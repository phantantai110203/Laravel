...
<form method="post" action="{{route('products.update',['product'=>$p])}}" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    <label> Tên sản phẩm:</label><input name="name" value="{{old('name',$p->name)}}"><br>
    @if($errors->has('name')){{$errors->first('name')}}<br> @endif
    <label> Giá:</label><input name="price" value="{{old('price',$p->price)}}"><br>
    ...
    <label> Hình:</label>
    <img style="width:100px;max-height:100px;object-fit:contain;" src="{{$p->image}}"><br>
