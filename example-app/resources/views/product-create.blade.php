...
<form method="post" action="{{route('products.store')}}" enctype="multipart/form-data">
    @csrf
    <label> Tên sản phẩm:</label><input name="name" value="{{old('name')}}"><br>
    @if($errors->has('name')){{$errors->first('name')}}<br> @endif
    <label> Giá:</label><input name="price" value="{{old('price')}}"><br>
    @if($errors->has('price')){{$errors->first('price')}}<br> @endif
    <label> Loại sản phẩm</label>
    <select name=" category">
        <option value=''> --Chọn loại--</option>
        @foreach($lst as $cat)
        <option value="{{$cat->id}}" @if ($cat->id==old('category'))
            selected @endif>{{$cat->name}} </option>
        @endforeach
    </select><br>
    @if($errors->has('category')){{$errors->first('category')}}<br> @endif
    <label> Mô tả:</label><textarea name="desc">{{old('desc')}}</textarea><br>
